...
void UShooterAnimInstance::NativeUpdateAnimation(float DeltaTime)
{
	...
	FRotator DeltaRot = UKismetMathLibrary::NormalizedDeltaRotator(MovementRotation, AimRotation); // For Smoother Movement
	...
	// Recoil
	if (Cast<AFirearmWeapon>(EquippedWeapon))
	{
		AFirearmWeapon* EquippedGun = Cast<AFirearmWeapon>(EquippedWeapon);
...
int UShooterAnimInstance::CalcWeaponIdx()
{
