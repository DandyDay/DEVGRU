...
void UWeaponActionComponent::StartFireTimer()
{
	...
	OwnerShooterCharacter->GetWorldTimerManager().SetTimer(
		...
		OwnerShooterCharacter->GetEquippedWeapon()->FireDelay
		);
}
...
{
	bCanFire = true;
	if (bFireButtonPressed && Cast<AFirearmWeapon>(OwnerShooterCharacter->GetEquippedWeapon()) && Cast<AFirearmWeapon>(OwnerShooterCharacter->GetEquippedWeapon())->bAutomatic)
	{
		Fire();
	}
}
...
{
	if (Cast<AFirearmWeapon>(OwnerShooterCharacter->GetEquippedWeapon()))
		Cast<AFirearmWeapon>(OwnerShooterCharacter->GetEquippedWeapon())->PlayReloadAnim();
	if (OwnerShooterCharacter->GetEquippedWeapon()->GetWeaponType() == EWeaponType::EWT_Grenade)
...
float UWeaponActionComponent::CalcHandSwayPitch()
{
	...
	OwnerShooterCharacter->GetMoveX() + SpringValue * OwnerShooterCharacter->GetEquippedWeapon()->SpringPitchMultiplier));
}
...
float UWeaponActionComponent::CalcHandSwayYaw()
{
	...
	return (FMath::GetMappedRangeValueClamped(FVector2d(-1.f, 1.f), OutRange, 
	// OwnerShooterCharacter->GetMouseX() + SpringValue * OwnerShooterCharacter->GetEquippedWeapon()->SpringYawMultiplier));
	SpringValue * OwnerShooterCharacter->GetEquippedWeapon()->SpringYawMultiplier));
}
...
float UWeaponActionComponent::CalcHandSwayRoll()
{
	...
	return (FMath::GetMappedRangeValueClamped(FVector2d(-1.f, 1.f), OutRange, 
	// OwnerShooterCharacter->GetMouseY() + SpringValue * OwnerShooterCharacter->GetEquippedWeapon()->SpringRollMultiplier));
	SpringValue * OwnerShooterCharacter->GetEquippedWeapon()->SpringRollMultiplier));
}
...
void UWeaponActionComponent::SetHUDCrosshairs(float DeltaTime)
{
	...
	if (OwnerShooterController)
	{
		...
		if (HUD)
		{
			if (OwnerShooterCharacter->GetEquippedWeapon() && Cast<AFirearmWeapon>(OwnerShooterCharacter->GetEquippedWeapon()))
			{
				AFirearmWeapon* EquippedGun = Cast<AFirearmWeapon>(OwnerShooterCharacter->GetEquippedWeapon());
			...
			if (bAiming)
			{
				CrosshairAimFactor = FMath::FInterpTo(CrosshairAimFactor, 2.0f, DeltaTime, 30.f);
			}
			else
			{
