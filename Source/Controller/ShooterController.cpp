...
void AShooterController::ServerCheckMatchState_Implementation()
{
	AShooterGameMode* GameMode = Cast<AShooterGameMode>(UGameplayStatics::GetGameMode(this));
	if (GameMode)
	{
		WarmupTime = GameMode->GetWarmpupTime();
...
// Also Called in server
void AShooterController::ClientJoinMidgame_Implementation(FName StateOfMatch, float Warmup, float Match, float Cooldown,float StartingTime)
{
	WarmupTime = Warmup;
	MatchTime = Match;
...
void AShooterController::SetupInputComponent()
{
	...
	// InputComponent->BindAction("SpectateNext", IE_Pressed, this, &AShooterController::SpectateNextPlayer);
	// InputComponent->BindAction("SpectatePrev", IE_Pressed, this, &AShooterController::SpectatePrevPlayer);
}
